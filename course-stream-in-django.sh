...
clear
echo -e "			${BROWN}#############################################${NC}"
echo -e "			${BROWN}#          Course Stream in Django          #${NC}"
echo -e "			${BROWN}#-------------------------------------------#${NC}"
...
if [ -x course-stream-in-django.sh ]
then
	cd ~/Downloads/
	filename=course-stream-in-django
	if [ -e $filename ]
	then
		echo -e "${RED}Repo already exit.${NC}"
		rm -rf course-stream-in-django
		if [ $? -eq 0 ]
		then
			echo -e "${GREEN}Repo Deleted.${NC}"
			echo -e "${YELLOW}Downloading ${NC}${BROWN}course-stream-in-django ${NC}${YELLOW}files....${NC}"
	...
	then
		...
		#moveing course-stream-in-django repo
