...
clear
echo -e "		${BROWN}#############################################################${NC}"
echo -e "		${BROWN}#            Sentiment Analysis of Restaurant Review        #${NC}"
echo -e "		${BROWN}#-----------------------------------------------------------#${NC}"
...
if [ -x Restaurant-review-sentiment-analysis.sh ]
then
	cd ~/Downloads/
	filename=Restaurant-Review-Sentiment-Analysis
	if [ -e $filename ]
	then
		echo -e "${RED}Repo already exit.${NC}"
		rm -rf Restaurant-Review-Sentiment-Analysis
		if [ $? -eq 0 ]
		then
			echo -e "${GREEN}Repo Deleted.${NC}"
			echo -e "${YELLOW}Downloading ${NC}${BROWN}Restaurant-Review-Sentiment-Analysis ${NC}${YELLOW}files....${NC}"
	...
	then
		...
		#moveing Restaurant-Review-Sentiment-Analysis repo
